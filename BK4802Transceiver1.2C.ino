/*1.2C无线电管理委员会验机合规版*/
//该版本为业余无线电自制电台验机时刷写使用
//更新日期：2026年7月28日  编写人：SG
//修订日期：2026年7月29日
/*其实是因为第一次去无委时候被打回来了说是V段频率有6kHz的偏移
才紧急修改的这一版，这一版包过，不过的话别怪我，要怪就怪你们市无委
反正我是过了，50兆，144兆，430兆都过了，那个⻌畐还说我这个功率太低
反正测的时候衰减都调最小了都还不起表，反正也不戳用担心*/
//电台功能：仅保留50M，144M，430M三个业余段
/*特别注意：
某些批次可能由于元件离散性导致杂散增大和略微的频率偏移（都怪嘉立创）
不同的机子可能偏移的方向，大小都不一样，
建议在验机之前自己上频谱仪测试一下
如果遇到频偏超过标准的情况，
请修改函数uint32_t compFreq(uint32_t freq)的修正系数*/
...
#define NUM_BANDS   3           // 频段数量（只保留3个业余段）
...
Band bands[NUM_BANDS] = {
    ...
    { 50000, 54000, 51500, 51500, "F1", "VHF" },  // 6米波段
    { 144000, 148000, 145000, 145000, "F2", "VHF" }, // 2米波段
    //{ 409000, 410000, 409700, 409700, "F4", "UHF" }, // 公众段
    { 430000, 440000, 438500, 438500, "F3", "UHF" }  // 业余U段
};
...
uint32_t compFreq(uint32_t freq) {
    if (currentBand == 2) {
        return freq + 10;   // U段补偿，使实际收发频率与显示一致
    }
...
void displayInitCaption() {
    ...
    display.setTextSize(2);
    display.print(F("V1.2C"));//版本号
    display.setCursor(25, 55);
    display.setTextSize(1);
    display.print(F("Compliant version"));//小标识
    display.display();
    delay(3000);
}// 显示初始化信息
